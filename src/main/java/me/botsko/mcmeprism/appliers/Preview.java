...
import me.botsko.mcmeprism.events.PrismBlocksRollbackEvent;
...
public class Preview implements Previewable {
     ...
     * @return
     */
    public Preview(MCMEPrism plugin, CommandSender sender, List<Handler> results, QueryParameters parameters,
    ...
    public void cancel_preview() {
        ...
        sender.sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Preview canceled." + ChatColor.GRAY
                + " Please come again!" ) );
    }
    ...
    public void apply_preview() {
        if( player == null )
            return;
        sender.sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Applying rollback from preview..." ) );
        setIsPreview( false );
        ...
        if( !worldChangeQueue.isEmpty() ) {
            ...
            if( !is_preview && player != null ) {
                ...
                if( MCMEPrism.playersWithActiveTools.containsKey( player.getName() ) ) {
                    ...
                    oldwand = MCMEPrism.playersWithActiveTools.get( player.getName() );
                }
        ...
        worldChangeQueueTaskId = plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
            ...
            public void run() {
                ...
                if( plugin.getConfig().getBoolean( "prism.debug" ) ) {
                    MCMEPrism.debug( "World change queue size: " + worldChangeQueue.size() );
                }

                if( worldChangeQueue.isEmpty() ) {
                    sender.sendMessage(MCMEPrism.messenger.playerError( ChatColor.GRAY
                ...
                if( currentQueueOffset < worldChangeQueue.size() ) {
                    for ( final Iterator<Handler> iterator = worldChangeQueue.listIterator( currentQueueOffset ); iterator
                        ...
                        try {
                            ...
                            MCMEPrism.log( "Applier error: " + e.getMessage() );
                            e.printStackTrace();
    ...
    public void fireApplierCallback() {
        ...
        if( plugin.getConfig().getBoolean( "prism.debug" ) ) {
            ...
            MCMEPrism.debug( "Changes: " + changes_applied_count );
            MCMEPrism.debug( "Planned: " + changes_planned_count );
            MCMEPrism.debug( "Skipped: " + skipped_block_count );
        }
    }
}
...
public class OreMonitor {
     ...
     * @param plugin
     */
    public OreMonitor(MCMEPrism plugin) {
        this.plugin = plugin;
    }
    ...
    protected String getOreColor(Block block) {
        if( isWatched( block ) ) {
            return MCMEPrism.getAlertedOres().get( "" + block.getTypeId() );
        } else {
            return "&f";
        }
    }
    ...
    protected boolean isWatched(Block block) {
        return MCMEPrism.getAlertedOres().containsKey( block.getTypeId() + ":" + block.getData() )
                || MCMEPrism.getAlertedOres().containsKey( "" + block.getTypeId() );
    }

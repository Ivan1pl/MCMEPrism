package me.botsko.mcmeprism.bridge;
...
public class WorldEditBridge {
     ...
     * @return
     */
    public static boolean getSelectedArea(MCMEPrism plugin, Player player, QueryParameters parameters) {
        // Get selected area
        Region region;
        try {
            final LocalPlayer lp = new BukkitPlayer( MCMEPrism.plugin_worldEdit, MCMEPrism.plugin_worldEdit.getWorldEdit()
                    .getServer(), player );
            final LocalWorld lw = lp.getWorld();
            region = MCMEPrism.plugin_worldEdit.getWorldEdit().getSession( lp ).getSelection( lw );
        ...
        final Selection sel = MCMEPrism.plugin_worldEdit.getSelection( player );

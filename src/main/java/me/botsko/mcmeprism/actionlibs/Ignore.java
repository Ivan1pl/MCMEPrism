...
public class Ignore {
    ...
    public Ignore(MCMEPrism plugin) {
        this.plugin = plugin;
    ...
    public boolean event(String actionTypeName, Player player) {
        ...
        if( plugin.getConfig().getBoolean( "prism.ignore.enable-perm-nodes" )
            ...
            MCMEPrism.debug( "Player has permission node to ignore " + actionTypeName );
            return false;
        }
    ...
    public boolean event(Player player) {
        ...
        if( player == null || player.getName() == null ) {
            MCMEPrism.debug( "Player is being ignored, name is null" );
            return false;
        }

        // Should we ignore this player?
        if( ignore_players != null && ignore_players.contains( player.getName() ) ) {
            MCMEPrism.debug( "Player is being ignored, per config: " + player.getName() );
            return false;
        }
        ...
        if( ignore_creative ) {
            if( player.getGameMode().equals( GameMode.CREATIVE ) ) {
                MCMEPrism.debug( "Player is in creative mode, creative mode ignored: " + player.getName() );
                return false;
            }
        }
    ...
    public boolean event(String actionTypeName, World world) {
        ...
        if( ignore_worlds != null && ignore_worlds.contains( world.getName() ) ) {
            MCMEPrism.debug( "World is being ignored, per config: " + world.getName() );
            return false;
        }

...
public class PrismBlockEvents implements Listener {
     ...
     * @param plugin
     */
    public PrismBlockEvents(MCMEPrism plugin) {
        this.plugin = plugin;
    }
    ...
    protected void logBlockRelationshipsForBlock(String playername, Block block) {
        ...
        final ArrayList<Block> falling_blocks = us.dhmc.mcmeelixr.BlockUtils.findFallingBlocksAboveBlock( block );
        ...
        if( block.getType().equals( Material.PISTON_EXTENSION )
            ...
            final ArrayList<Block> pistonBases = us.dhmc.mcmeelixr.BlockUtils.findSideFaceAttachedBlocks( block );
        ...
        ArrayList<Block> detached_blocks = us.dhmc.mcmeelixr.BlockUtils.findSideFaceAttachedBlocks( block );
        ...
        detached_blocks = us.dhmc.mcmeelixr.BlockUtils.findTopFaceAttachedBlocks( block );
        if( detached_blocks.size() > 0 ) {
            for ( final Block b : detached_blocks ) {
                RecordingQueue.addToQueue( ActionFactory.createBlock("block-break", b, playername) );
            }
        ...
        final ArrayList<Entity> hanging = us.dhmc.mcmeelixr.BlockUtils.findHangingEntities( block );
        if( hanging.size() > 0 ) {
    ...
    public void onBlockBreak(final BlockBreakEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "block-break", player ) )
            return;
        ...
        if( block.getType().equals( Material.OBSIDIAN ) ) {
            final ArrayList<Block> blocks = us.dhmc.mcmeelixr.BlockUtils.findConnectedBlocksOfType( Material.PORTAL,
                    block, null );
            if( !blocks.isEmpty() ) {
    ...
    public void onBlockPlace(final BlockPlaceEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "block-place", player ) )
            return;
    ...
    public void onBlockSpread(final BlockSpreadEvent event) {
        ...
        if( event.getNewState().getType().equals( Material.FIRE ) ) {
            if( !MCMEPrism.getIgnore().event( "fire-spread" ) )
                return;
            type = "fire-spread";
        } else {
            if( !MCMEPrism.getIgnore().event( "block-spread", event.getBlock() ) )
                return;
        }
    ...
    public void onBlockForm(final BlockFormEvent event) {
        if( !MCMEPrism.getIgnore().event( "block-form", event.getBlock() ) )
            return;
        final Block b = event.getBlock();
        final BlockState s = event.getNewState();
    ...
    public void onBlockFade(final BlockFadeEvent event) {
        if( !MCMEPrism.getIgnore().event( "block-fade", event.getBlock() ) )
            return;
        final Block b = event.getBlock();
    ...
    public void onLeavesDecay(final LeavesDecayEvent event) {
        if( !MCMEPrism.getIgnore().event( "leaf-decay", event.getBlock() ) )
            return;
    ...
    public void onBlockBurn(final BlockBurnEvent event) {
        if( !MCMEPrism.getIgnore().event( "block-burn", event.getBlock() ) )
            return;
        Block block = event.getBlock();
    ...
    public void onSignChange(final SignChangeEvent event) {
        if( !MCMEPrism.getIgnore().event( "sign-change", event.getPlayer() ) )
            return;
    ...
    public void onSetFire(final BlockIgniteEvent event) {
        ...
        if( cause != null ) {
            ...
            if( !MCMEPrism.getIgnore().event( cause, event.getBlock().getWorld() ) )
                return;
    ...
    public void onBlockDispense(final BlockDispenseEvent event) {
        if( !MCMEPrism.getIgnore().event( "block-dispense" ) )
            return;
    ...
    public void onPistonExtend(final BlockPistonExtendEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "block-shift", event.getBlock() ) )
            return;
    ...
    public void onBlockFromTo(final BlockFromToEvent event) {
        ...
        if( us.dhmc.mcmeelixr.BlockUtils.canFlowBreakMaterial( to.getType() ) ) {
            if( from.getType() == Material.STATIONARY_WATER || from.getType() == Material.WATER ) {
                if( MCMEPrism.getIgnore().event( "water-break", event.getBlock() ) ) {
                ...
                if( MCMEPrism.getIgnore().event( "lava-break", event.getBlock() ) ) {
        ...
        // Record water flow
        if( from.getType() == Material.STATIONARY_WATER || from.getType() == Material.WATER ) {
            if( MCMEPrism.getIgnore().event( "water-flow", event.getBlock() ) ) {
        ...
        if( from.getType() == Material.STATIONARY_LAVA || from.getType() == Material.LAVA ) {
            if( MCMEPrism.getIgnore().event( "lava-flow", event.getBlock() ) ) {

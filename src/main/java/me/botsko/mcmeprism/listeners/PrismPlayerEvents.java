...
public class PrismPlayerEvents implements Listener {
    ...
    public PrismPlayerEvents(MCMEPrism plugin) {
        this.plugin = plugin;
    ...
    public void onCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        if( plugin.getConfig().getBoolean( "prism.alerts.illegal-commands.enabled" ) ) {
            if( illegalCommands.contains( primaryCmd ) ) {
                final String msg = player.getName() + " attempted an illegal command: " + primaryCmd + ". Originally: "
                        + cmd;
                player.sendMessage(MCMEPrism.messenger.playerError( "Sorry, this command is not available in-game." ) );
        ...
        if( !MCMEPrism.getIgnore().event( "player-command", player ) )
            return;
    ...
    public void onPlayerJoin(final PlayerJoinEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "player-join", player ) )
            return;
    ...
    public void onPlayerQuit(final PlayerQuitEvent event) {
        ...
        MCMEPrism.prismPlayers.remove( event.getPlayer().getName() );

        // Track player quit
        if( !MCMEPrism.getIgnore().event( "player-quit", event.getPlayer() ) )
            return;
        ...
        if( MCMEPrism.playersWithActiveTools.containsKey( event.getPlayer().getName() ) ) {
            MCMEPrism.playersWithActiveTools.remove( event.getPlayer().getName() );
        }
    ...
    public void onPlayerChat(final AsyncPlayerChatEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "player-chat", event.getPlayer() ) )
            return;
    ...
    public void onPlayerDropItem(final PlayerDropItemEvent event) {
        if( !MCMEPrism.getIgnore().event( "item-drop", event.getPlayer() ) )
            return;
    ...
    public void onPlayerPickupItem(final PlayerPickupItemEvent event) {
        if( !MCMEPrism.getIgnore().event( "item-pickup", event.getPlayer() ) )
            return;
    ...
    public void onPlayerExpChangeEvent(final PlayerExpChangeEvent event) {
        if( !MCMEPrism.getIgnore().event( "xp-pickup", event.getPlayer() ) )
            return;
    ...
    public void onPlayerBucketEmpty(final PlayerBucketEmptyEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( cause, player ) )
            return;
    ...
    public void onPlayerBucketFill(final PlayerBucketFillEvent event) {
        ...
        final Player player = event.getPlayer();
        if( !MCMEPrism.getIgnore().event( "bucket-fill", player ) )
            return;
        final Block spot = event.getBlockClicked().getRelative( event.getBlockFace() );
    ...
    public void onPlayerTeleport(final PlayerTeleportEvent event) {
        if( !MCMEPrism.getIgnore().event( "player-teleport", event.getPlayer() ) )
            return;
    ...
    public void onEnchantItem(final EnchantItemEvent event) {
        if( !MCMEPrism.getIgnore().event( "enchant-item", event.getEnchanter() ) )
            return;
    ...
    public void onCraftItem(final CraftItemEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "craft-item", player ) )
            return;
    ...
    public void onPlayerInteract(final PlayerInteractEvent event) {
        ...
        if( MCMEPrism.playersWithActiveTools.containsKey( player.getName() ) ) {
            ...
            if( wand != null && player.getItemInHand().getTypeId() == item_id
                ...
                if( ( event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.LEFT_CLICK_BLOCK ) ) {
                    MCMEPrism.debug( "Cancelling event for wand use." );
                    event.setCancelled( true );
        ...
        if( block != null && event.getAction() == Action.RIGHT_CLICK_BLOCK ) {
            ...
            switch ( block.getType() ) {
                ...
                case HOPPER:
                case DROPPER:
                    if( !MCMEPrism.getIgnore().event( "container-access", player ) )
                        return;
                ...
                case STONE_BUTTON:
                case WOOD_BUTTON:
                    if( !MCMEPrism.getIgnore().event( "block-use", player ) )
                        return;
                ...
                case TNT:
                    if( player.getItemInHand().getType().equals( Material.FLINT_AND_STEEL ) ) {
                        if( !MCMEPrism.getIgnore().event( "tnt-prime", player ) )
                            return;
        ...
        if( block != null && event.getAction() == Action.PHYSICAL ) {
            if( block.getType() == Material.SOIL ) { // They are stepping on
                                                     // soil
                if( !MCMEPrism.getIgnore().event( "crop=trample", player ) )
                    return;
    ...
    protected void recordCocoaPlantEvent(Block block, ItemStack inhand, BlockFace clickedFace, String player) {
        if( !MCMEPrism.getIgnore().event( "block-place", block ) )
            return;
    ...
    protected void recordBonemealEvent(Block block, ItemStack inhand, BlockFace clickedFace, String player) {
        if( inhand.getTypeId() == 351 && inhand.getDurability() == 15 ) {
            if( !MCMEPrism.getIgnore().event( "bonemeal-use", block ) )
                return;
    ...
    protected void recordMonsterEggUse(Block block, ItemStack inhand, String player) {
        if( !MCMEPrism.getIgnore().event( "spawnegg-use", block ) )
            return;
    ...
    protected void recordRocketLaunch(Block block, ItemStack inhand, BlockFace clickedFace, String player) {
        if( !MCMEPrism.getIgnore().event( "firework-launch", block ) )
            return;
        RecordingQueue
    ...
    protected void recordCakeEat(Block block, Player player) {
        if( !MCMEPrism.getIgnore().event( "cake-eat", block ) )
            return;

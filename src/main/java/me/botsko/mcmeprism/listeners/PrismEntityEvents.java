...
import us.dhmc.mcmeelixr.DeathUtils;
...
import me.botsko.mcmeprism.utils.WandUtils;
...
public class PrismEntityEvents implements Listener {
     ...
     * @param plugin
     */
    public PrismEntityEvents(MCMEPrism plugin) {
        this.plugin = plugin;
    }
    ...
    public void onEntityDamageEvent(final EntityDamageByEntityEvent event) {
        ...
        if( entity instanceof ItemFrame ) {
            ...
            if( !frame.getItem().getType().equals( Material.AIR ) ) {
                if( MCMEPrism.getIgnore().event( "item-remove", player ) ) {
    ...
    public void onEntityDeath(final EntityDeathEvent event) {
        ...
        if( !( entity instanceof Player ) ) {
            if( entity.getLastDamageCause() instanceof EntityDamageByEntityEvent ) {
                ...
                if( entity instanceof Horse ) {
                    ...
                    if( horse.isCarryingChest() ) {
                        // Log item drops
                        if( MCMEPrism.getIgnore().event( "item-drop", entity.getWorld() ) ) {
                ...
                if( entityDamageByEntityEvent.getDamager() instanceof Player ) {
                    ...
                    if( !MCMEPrism.getIgnore().event( "player-kill", player ) )
                        return;
                ...
                // Mob shot by an arrow from a player
                    ...
                    if( name == null )
                        name = "unknown";
                    if( !MCMEPrism.getIgnore().event( "entity-kill", entity.getWorld() ) )
                        return;
            ...
            if( MCMEPrism.getIgnore().event( "player-death", p ) ) {
            ...
            // Log item drops
            if( MCMEPrism.getIgnore().event( "item-drop", p ) ) {
    ...
    public void onCreatureSpawn(final CreatureSpawnEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-spawn", event.getEntity().getWorld() ) )
            return;
    ...
    public void onEntityTargetEvent(final EntityTargetEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-follow", event.getEntity().getWorld() ) )
            return;
    ...
    public void onPlayerShearEntity(final PlayerShearEntityEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-shear", event.getPlayer() ) )
            return;
        RecordingQueue
    ...
    public void onPlayerInteractEntityEvent(final PlayerInteractEntityEvent event) {
        ...
        if( e instanceof ItemFrame ) {
            ...
            if( frame.getItem().getType().equals( Material.AIR ) && p.getItemInHand() != null ) {
                if( MCMEPrism.getIgnore().event( "item-insert", p ) ) {
    ...
    public void onEntityBreakDoor(final EntityBreakDoorEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-break", event.getEntity().getWorld() ) )
            return;
    ...
    public void onPlayerEntityLeash(final PlayerLeashEntityEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-leash", event.getPlayer() ) )
            return;
        RecordingQueue
    ...
    public void onPlayerEntityUnleash(final PlayerUnleashEntityEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-unleash", event.getPlayer() ) )
            return;
    ...
    public void onEntityUnleash(final EntityUnleashEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-unleash" ) )
            return;
    ...
    public void onPotionSplashEvent(final PotionSplashEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "potion-splash", player ) )
            return;
    ...
    public void onHangingPlaceEvent(final HangingPlaceEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "hangingitem-place", event.getPlayer() ) )
            return;
    ...
    public void onHangingBreakEvent(final HangingBreakEvent event) {
        ...
        if( !MCMEPrism.getIgnore().event( "hangingitem-break", event.getEntity().getWorld() ) )
            return;
        ...
        if( plugin.preplannedBlockFalls.containsKey( coord_key ) ) {
            ...
            if( !MCMEPrism.getIgnore().event( "item-remove", event.getEntity().getWorld() ) )
                return;
    ...
    public void onEntityChangeBlock(final EntityChangeBlockEvent event) {
        ...
        if( from == Material.GRASS && to == Material.DIRT) {
            if( event.getEntityType() != EntityType.SHEEP )
                return;
            if( !MCMEPrism.getIgnore().event( "sheep-eat", event.getBlock() ) )
                return;
            ...
            if (from == Material.AIR) {
                if (!MCMEPrism.getIgnore().event("enderman-place", event.getBlock()))
                    return;
                BlockState state = event.getBlock().getState();
                state.setType(to);
                RecordingQueue.addToQueue(ActionFactory.createBlock("enderman-place", state, entity));
            } else {
                if (!MCMEPrism.getIgnore().event("enderman-pickup", event.getBlock()))
                    return;
    ...
    public void onEntityBlockForm(final EntityBlockFormEvent event) {
        if( !MCMEPrism.getIgnore().event( "entity-form", event.getBlock() ) )
            return;
    ...
    public void onEntityExplodeChangeBlock(final EntityExplodeEvent event) {
        ...
        if( event.getEntity() != null ) {
            if( event.getEntity() instanceof Creeper ) {
                if( !MCMEPrism.getIgnore().event( "creeper-explode", event.getEntity().getWorld() ) )
                    return;
                action = "creeper-explode";
                name = "creeper";
            } else if( event.getEntity() instanceof TNTPrimed ) {
                if( !MCMEPrism.getIgnore().event( "tnt-explode", event.getEntity().getWorld() ) )
                    return;
                action = "tnt-explode";
                Entity source = ((TNTPrimed) event.getEntity()).getSource();
                name = followTNTTrail(source);
            } else if( event.getEntity() instanceof EnderDragon ) {
                if( !MCMEPrism.getIgnore().event( "dragon-eat", event.getEntity().getWorld() ) )
                    return;
                action = "dragon-eat";
                name = "enderdragon";
            } else {
                if( !MCMEPrism.getIgnore().event( "entity-explode", event.getLocation().getWorld() ) )
                    return;
                try {

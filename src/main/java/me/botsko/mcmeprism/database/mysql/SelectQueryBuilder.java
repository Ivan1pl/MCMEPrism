...
import me.botsko.mcmeprism.database.QueryBuilder;
...
public class SelectQueryBuilder extends QueryBuilder {
     ...
     * @param plugin
     */
    public SelectQueryBuilder(MCMEPrism plugin) {
        super( plugin );
        prefix = MCMEPrism.config.getString("prism.mysql.prefix");
    }
    ...
    protected void actionCondition() {
        ...
        for ( final Entry<String, Integer> entry : MCMEPrism.prismActions.entrySet() ) {
            if( entry.getKey().contains( "prism" ) ) {
                ...
                prismActionIds.add("" + MCMEPrism.prismActions.get( entry.getKey() ) );
            }
        }
        ...
        if( action_types.size() > 0 ) {
            ...
            for ( final Entry<String, MatchRule> entry : action_types.entrySet() ) {
                if( entry.getValue().equals( MatchRule.INCLUDE ) ) {
                    includeIds.add("" + MCMEPrism.prismActions.get( entry.getKey() ) );
                }
                if( entry.getValue().equals( MatchRule.EXCLUDE ) ) {
                    excludeIds.add("" + MCMEPrism.prismActions.get( entry.getKey() ) );
                }
            }
            // Include IDs

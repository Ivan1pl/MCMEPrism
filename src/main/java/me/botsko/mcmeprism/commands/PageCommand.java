...
public class PageCommand implements SubHandler {
     ...
     * @return
     */
    public PageCommand(MCMEPrism plugin) {
        this.plugin = plugin;
    }
    ...
    public void handle(CallInfo call) {
        ...
        if( call.getArgs().length != 2 ) {
            call.getSender().sendMessage(MCMEPrism.messenger.playerError( "Please specify a page number. Like /prism page 2" ) );
            return;
        }
        ...
        if( TypeUtils.isNumeric( call.getArg( 1 ) ) ) {
            ...
            // Previous page
                ...
                if( results.getPage() <= 1 ) {
                    call.getSender().sendMessage(MCMEPrism.messenger.playerError( "There is no previous page." ) );
                    return;
                }
                page = results.getPage() - 1;
            } else {
        ...
        // No negatives
        if( page <= 0 ) {
            call.getSender().sendMessage(MCMEPrism.messenger.playerError( "Page must be greater than zero." ) );
            return;
        }
        ...
        if( results.getActionResults().isEmpty() ) {
            call.getSender().sendMessage(MCMEPrism.messenger.playerError( "Nothing found." + ChatColor.GRAY
                            + " Either you're missing something, or we are." ) );
            return;
        }

        call.getSender().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Showing " + results.getTotalResults() + " results. Page " + page
        ...
        for ( final Handler a : paginated ) {
            ...
            call.getSender().sendMessage(MCMEPrism.messenger.playerMsg( am.getMessage() ) );
            result_count++;
        }
    }

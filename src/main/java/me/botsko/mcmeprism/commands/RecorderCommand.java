...
public class RecorderCommand implements SubHandler {
     ...
     * @return
     */
    public RecorderCommand(MCMEPrism plugin) {
        this.plugin = plugin;
    }
    ...
    public void handle(final CallInfo call) {
        ...
        if( call.getArgs().length <= 1 ) {
            call.getSender().sendMessage(MCMEPrism.messenger.playerError( "Invalid command. Use /pr ? for help" ) );
            return;
        }
        ...
        if( call.getArg( 1 ).equals( "cancel" ) ) {
            if( recorderActive ) {
                ...
                call.getSender().sendMessage(MCMEPrism.messenger.playerMsg( "Current recording task has been canceled." ) );
                call.getSender()
                        .sendMessage(MCMEPrism.messenger
                                        .playerError( "WARNING: Actions will collect until queue until recorder restarted manually." ) );
            } else {
                call.getSender().sendMessage(MCMEPrism.messenger.playerError( "No recording task is currently running." ) );
            }
            return;
        }
        ...
        if( call.getArg( 1 ).equals( "start" ) ) {
            if( recorderActive ) {
                call.getSender().sendMessage(MCMEPrism.messenger.playerError( "Recording tasks are currently running. Cannot start." ) );
            } else {

                // Run db tests...
                call.getSender().sendMessage(MCMEPrism.messenger.playerMsg( "Validating database connections..." ) );
                ...
                try {
                    ...
                    call.getSender().sendMessage(MCMEPrism.messenger.playerSuccess( "Valid connection found. Yay!" ) );

                    call.getSender().sendMessage(MCMEPrism.messenger.playerMsg( "Restarting recordering tasks..." ) );

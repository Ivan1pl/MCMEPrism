...
import me.botsko.mcmeprism.wands.InspectorWand;
import me.botsko.mcmeprism.wands.Wand;
import me.botsko.mcmeprism.wands.QueryWandBase;
import me.botsko.mcmeprism.wands.RollbackWand;
import me.botsko.mcmeprism.wands.RestoreWand;
import me.botsko.mcmeprism.wands.ProfileWand;
import me.botsko.mcmeprism.utils.InventoryUtils;
...
public class WandCommand implements SubHandler {
     ...
     * @return
     */
    public WandCommand(MCMEPrism plugin) {
        this.plugin = plugin;
    }
    ...
    public void handle(CallInfo call) {
        ...
        if( !isInspect ) {
            if( call.getArgs().length < 2 ) {
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerError( "You need to specify a wand type. Use '/prism ?' for help." ) );
                return;
            }
            type = call.getArg( 1 );
        }
        ...
        if( MCMEPrism.playersWithActiveTools.containsKey( call.getPlayer().getName() ) ) {
            ...
            oldwand = MCMEPrism.playersWithActiveTools.get( call.getPlayer().getName() );
        }
        ...
        String parameters = "";
        if( item_id != 0 ) {
            item_name = MCMEPrism.getItems().getAlias( item_id, item_subid );
        ...
        if( !ItemUtils.isAcceptableWand( item_id, item_subid ) ) {
            call.getPlayer().sendMessage(MCMEPrism.messenger.playerError( "Sorry, but you may not use " + item_name + " for a wand." ) );
            return;
        }
        ...
        if( type.equalsIgnoreCase( "i" ) || type.equalsIgnoreCase( "inspect" ) ) {
            ...
            if( oldwand != null && oldwand instanceof InspectorWand ) {
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Inspection wand " + ChatColor.RED + "disabled"
                                + ChatColor.WHITE + "." ) );
            } else {
                wand = new InspectorWand( plugin );
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Inspection wand " + ChatColor.GREEN + "enabled"
         ...
         */
            ...
            if( oldwand != null && oldwand instanceof ProfileWand ) {
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Profile wand " + ChatColor.RED + "disabled" + ChatColor.WHITE
                                + "." ) );
            } else {
                wand = new ProfileWand();
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Profile wand " + ChatColor.GREEN + "enabled"
         ...
         */
            ...
            if( oldwand != null && oldwand instanceof RollbackWand ) {
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Rollback wand " + ChatColor.RED + "disabled"
                                + ChatColor.WHITE + "." ) );
            } else {
                wand = new RollbackWand( plugin );
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Rollback wand " + ChatColor.GREEN + "enabled"
                                + ChatColor.WHITE + wandOn + parameters + "." ) );
                enabled = true;
            }
        }
         ...
         */
            ...
            if( oldwand != null && oldwand instanceof RestoreWand ) {
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Restore wand " + ChatColor.RED + "disabled" + ChatColor.WHITE
                                + "." ) );
            } else {
                wand = new RestoreWand( plugin );
                call.getPlayer().sendMessage(MCMEPrism.messenger.playerHeaderMsg( "Restore wand " + ChatColor.GREEN + "enabled"
                                + ChatColor.WHITE + wandOn + parameters + "." ) );
                enabled = true;
            }
        }
        ...
        // Not a valid wand
            ...
            call.getPlayer().sendMessage(MCMEPrism.messenger.playerError( "Invalid wand type. Use /prism ? for help." ) );
            return;
        }
        ...
        if( enabled ) {
            ...
            MCMEPrism.debug( "Wand activated for player - mode: " + mode + " Item:" + item_id + ":" + item_subid );
            ...
            if( plugin.getConfig().getBoolean( "prism.wands.auto-equip" ) ) {
                if( !InventoryUtils.moveItemToHand( inv, item_id, item_subid ) ) {
                    ...
                    if( InventoryUtils.handItemToPlayer( inv, new ItemStack( item_id, 1, item_subid ) ) ) {
                        ...
                        call.getPlayer().sendMessage(MCMEPrism.messenger.playerError( "Can't fit the wand item into your inventory." ) );
                    }
                }
            ...
            if( wand instanceof QueryWandBase ) {
                if( !( (QueryWandBase) wand ).setParameters( call.getPlayer(), call.getArgs(), ( isInspect ? 1 : 2 ) ) ) { // This
                                                                                                                           ...
                                                                                                                           // was
                                                                                                                           // successful
                    call.getPlayer().sendMessage(MCMEPrism.messenger.playerError( "Notice: Only some parameters used.." ) );
                }
            }

            // Store
            MCMEPrism.playersWithActiveTools.put( call.getPlayer().getName(), wand );
        } else {
            if( oldwand != null ) {

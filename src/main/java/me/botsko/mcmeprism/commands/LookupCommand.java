...
public class LookupCommand implements SubHandler {
     ...
     * @return
     */
    public LookupCommand(MCMEPrism plugin) {
        this.plugin = plugin;
    }
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, new Runnable() {
            ...
            public void run() {
                ...
                for ( final CommandSender player : parameters.getSharedPlayers() ) {
                    ...
                    if( !isSender ) {
                        player.sendMessage(MCMEPrism.messenger.playerHeaderMsg( ChatColor.YELLOW + "" + ChatColor.ITALIC
                                + call.getSender().getName() + ChatColor.GOLD
                                + " shared these Prism lookup logs with you:" ) );
                    } else if( !sharingWithPlayers.isEmpty() ) {
                        player.sendMessage(MCMEPrism.messenger.playerHeaderMsg( ChatColor.GOLD
                    ...
                    if( !results.getActionResults().isEmpty() ) {
                        ...
                        if( !defaultsReminder.isEmpty() && isSender ) {
                            player.sendMessage(MCMEPrism.messenger.playerSubduedHeaderMsg( defaultsReminder ) );
                        }
                        ...
                        if( isSender ) {
                            player.sendMessage(MCMEPrism.messenger.playerError( "Nothing found." + ChatColor.GRAY
